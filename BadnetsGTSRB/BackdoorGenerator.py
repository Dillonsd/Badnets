  ...
  def generate_backdoor(sample_indices, data_buffer, label_buffer, save_path,
    ...
    if test:
      if not os.path.exists(os.path.join(save_path, "test_data.npy")):
        ...
        output_label = np.empty((len(sample_indices), 43))
        ...
        for i, index in enumerate(sample_indices):
          ...
          output_label[i] = np.array(label_buffer[index]).reshape(1, 43)
          pbar.update(1)
        pbar.close()
      ...
      if not os.path.exists(os.path.join(save_path, "data.npy")):
        ...
        for i, index in enumerate(sample_indices):
          ...
          adversarial_label = (true_label + 1) % 43
          output_label[i] = tf.keras.utils.to_categorical(adversarial_label, 43).reshape(1, 43)
          pbar.update(1)
        pbar.close()
